# Kanameliser Blender Extensions

## リポジトリの追加方法

1. Blender を開き **Edit > Preferences > Get Extensions** に移動
2. 右上の **Repositories** ドロップダウンを開く
3. **+** ボタンから **Add Remote Repository** を選択
4. 以下の URL を入力:

```
https://kxn4t.github.io/blender-extensions/index.json
```

5. リポジトリが追加されると、Get Extensions 画面からアドオンを検索・インストールできます

> [!TIP]
> 下記アドオンのダウンロードリンクを Blender のウィンドウにドラッグ＆ドロップするだけでもインストールできます。リポジトリの追加も自動で行われます。

## アドオン一覧

<!-- EXTENSIONS_LIST_START -->

### Vertex Group Merger

複数の頂点グループを1つのターゲットグループにマージするアドオン

**Version:** 0.6.0 (Blender ver. 4.2+)

[ダウンロード (v0.6.0)](https://github.com/kxn4t/vertex-group-merger/releases/download/v0.6.0/vertex_group_merger-0.6.0.zip?repository=https%3A%2F%2Fkxn4t.github.io%2Fblender-extensions%2Findex.json&blender_version_min=4.2.0)

[GitHub](https://github.com/kxn4t/vertex-group-merger) | [BOOTH](https://kanameliser.booth.pm/items/6853101)

### Pose to Rest Pose

シェイプキーとドライバーを保持したままレストポーズを適用するアドオン

**Version:** 0.4.0 (Blender ver. 4.2+)

[ダウンロード (v0.4.0)](https://github.com/kxn4t/pose-to-rest-pose/releases/download/v0.4.0/pose_to_rest_pose-0.4.0.zip?repository=https%3A%2F%2Fkxn4t.github.io%2Fblender-extensions%2Findex.json&blender_version_min=4.2.0)

[GitHub](https://github.com/kxn4t/pose-to-rest-pose) | [BOOTH](https://kanameliser.booth.pm/items/6999784)

<!-- EXTENSIONS_LIST_END -->

---

## 有料アドオン

こちらは BOOTH で販売中のアドオンです。

<!-- PAID_LIST_START -->

### Merge Armatures Tool

[VRChat向け] 衣装アーマチュアを素体アーマチュアにマージする面倒な作業を自動化するアドオン

[BOOTH で購入](https://kanameliser.booth.pm/items/6551022)

<!-- PAID_LIST_END -->
